...
"""Tests for repository commands with unique coverage.

This file tests repository commands that don't have dedicated test files:
- `repository cli` / `invenio` / `shell`
- `repository translations` / `index rebuild` / `reset` / `info`

Commands with dedicated test files (and removed from here to avoid duplication):
- lint/format/check → test_repository_lint_format.py + test_repository_check.py
- jslint/jstest → test_repository_jslint_jstest.py
- test → test_repository_test.py
