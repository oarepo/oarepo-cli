...
"""Unit tests for the top-level `repo-install` command (`cli/installer.py`).
...
def test_repo_install_uses_defaults() -> None:
    """Only REPOSITORY_NAME given: python/template/version/uv/uvx all take their defaults."""
    result = runner.invoke(app, ["repo-install", "my-repo"])
...
def test_repo_install_passes_all_options(tmp_path: Path) -> None:
...
def test_repo_install_requires_repository_name() -> None:
    """Omitting REPOSITORY_NAME entirely is a usage error (Click's own missing-argument handling)."""
    result = runner.invoke(app, ["repo-install"])

    assert result.exit_code == 2


def test_repo_install_rejects_blank_repository_name() -> None:
...
def test_repo_install_reports_missing_uv(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_repo_install_reports_missing_uvx(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_repo_install_reports_missing_python(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_repo_install_rejects_missing_config_file() -> None:
    """--config must point at an existing, readable file (Typer's own exists=True check)."""
    result = runner.invoke(app, ["repo-install", "--config", "does-not-exist.yaml", "my-repo"])

    assert result.exit_code == 2


def test_repo_install_help_lists_all_options() -> None:
    """--help documents every option and the positional repository_name argument."""
    result = runner.invoke(app, ["repo-install", "--help"])
