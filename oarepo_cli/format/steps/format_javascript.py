...
class FormatJavascriptStep(BaseFormatStep):
    ...
    def format_jsx(self, dirs, exclude):
        ...
        files = [f for f in self.find_files_with_extensions(dirs, "jsx", "tsx", "js", "ts")]
        if not files:
            return
        # copy .eslintrc.json into invenio
        shutil.copy(site.site_dir / '.eslintrc.json', site.invenio_instance_path / "assets" / '.eslintrc.json')
        for chunk in batched(files, 50):
            run_cmdline(site.invenio_instance_path / "assets" / "node_modules" / ".bin" / "eslint",
                        "-c", '.eslintrc.json',
                        "--fix", *chunk, cwd=assets_dir)
            run_cmdline(site.invenio_instance_path / "assets" / "node_modules" / ".bin" / "prettier", "--write", *chunk, cwd=assets_dir)
