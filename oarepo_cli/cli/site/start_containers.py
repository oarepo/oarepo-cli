...
If this step fails, please fix the problem and run the wizard again.            
    ...
    def check_redis(self):
        host, port = self.get_invenio_configuration('INVENIO_REDIS_HOST', 'INVENIO_REDIS_PORT')
    ...
    def check_db(self):
        host, port, user, password, dbname = self.get_invenio_configuration('INVENIO_DATABASE_HOST', 'INVENIO_DATABASE_PORT',
                                                                     'INVENIO_DATABASE_USER', 'INVENIO_DATABASE_PASSWORD',
    ...
    def check_mq(self):
        host, port = self.get_invenio_configuration('INVENIO_RABBIT_HOST', 'INVENIO_RABBIT_PORT')
    ...
    def check_search(self):
        (host, port, prefix,
            use_ssl, verify_certs, ssl_assert_hostname,
            ssl_show_warn
         ) = self.get_invenio_configuration('INVENIO_OPENSEARCH_HOST',
        ...
        try:
            ca_certs = self.get_invenio_configuration('INVENIO_OPENSEARCH_CA_CERTS_PATH')[0]
        except:
            ca_certs = None
        client = OpenSearch(
