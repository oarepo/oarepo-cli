    ...
    def check_version(self, *args, expected_major, expected_minor=None, strict=False):
        ...
        if strict:
            if isinstance(expected_major, (list, tuple)):
                assert major in expected_major, f"Expected major version to be one of {expected_major}, found {major}"
            else:
                assert major == expected_major, f"Expected major version to be one {expected_major}, found {major}"
                if expected_minor:
                    assert minor == expected_minor
        elif not (major > expected_major or (major == expected_major and minor >= expected_minor)):
            raise Exception('Expected docker compose version ')


    def _check_docker_compose_version(self, expected_major, expected_minor):
        self.check_version("docker-compose", "--version", expected_major=expected_major, expected_minor=expected_minor)

    def _check_node_version(self, *supported_versions):
        self.check_version("node", "--version", expected_major=supported_versions, strict=True)

    def _check_npm_version(self, *supported_versions):
        self.check_version("npm", "--version", expected_major=supported_versions, strict=True)
